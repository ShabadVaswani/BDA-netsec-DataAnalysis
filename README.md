...
## ⚙️ Current Data Platform

The project now supports two operational modes:

1. **Legacy Batch Mode (CSV-first)** for existing notebooks and historical reproducibility.
2. **Modern Streaming Mode (Kafka + Spark + Cassandra)** for real-time ingestion and fusion.

### Streaming Data Flow
- **Ingestion Producers**:
  - `src/download_routersense_data.js` -> Kafka RouterSense topic
  - `src/parse_garmin_complete.py` -> Kafka Garmin topic
  - `src/download_weather_data.py` -> Kafka Weather topic
- **Fusion**:
  - `src/spark/stream_fusion.py` performs structured stream parsing/joining and writes fused minute-level records.
- **Storage + Serving**:
  - Cassandra stores time-series features (`src/store/cassandra_schema.cql`).
  - Optional additive exports are written under `output/streaming/exports/`.
- **Fallback Compatibility**:
  - Existing CSV-based scripts remain available and are not removed.
...
├── src/
│   ├── download_*.js/.py          # Ingestion scripts
│   ├── parse_garmin_*.js/.py      # FIT parsing + normalization
│   ├── merge_*.py                 # Legacy batch merge utilities
│   ├── streaming/                 # Kafka config + producer helpers
│   ├── spark/                     # Spark Structured Streaming jobs
│   └── store/                     # Cassandra schema/init/export adapters
├── EXO-model/                     # Bio-exclusive VAE assets
├── LSTM model/                    # Sequence generation + LSTM artifacts
├── EXO-Hypermind/                 # PSI model + visualizations
├── AEON wellness index/           # Dashboard script + outputs
├── docs/                          # Operational and analysis documentation
├── data/                          # Local raw inputs (gitignored)
├── output/                        # Local outputs/models/exports (gitignored)
├── docker-compose.yml             # Kafka + Spark + Cassandra stack
...
## 🚀 Setup (Latest Software Baseline)

### Recommended Versions
- **Docker Desktop** with Compose v2 (latest stable)
- **Node.js** 20 LTS or newer
- **Python** 3.11+ (3.12 supported where dependencies allow)
- **Java** not required locally when running Spark via containers

### Install Dependencies
```bash
# Node dependencies (Playwright + Kafka client)
npm install

# Python dependencies (ML + Spark + Cassandra + Kafka)
...
```
Then set your RouterSense PID and any local endpoint overrides.

---

## 🔄 Run Modes

### A) Streaming Mode (Kafka + Spark + Cassandra) - Recommended
```bash
# 1) Start services
npm run stack:up
...
# 3) Publish source events
npm run stream:routersense -- 2025-11-18 2025-11-18
npm run stream:garmin
npm run stream:weather

# 4) Start Spark fusion
python src/spark/stream_fusion.py

# 5) Optional additive export
python src/store/export_cassandra.py --output output/streaming/exports/minute_features_v1.csv
```

To run compatible analysis scripts with Cassandra-backed exports first:
```bash
# PowerShell
...
### B) Legacy Batch Mode (CSV Pipeline)
```bash
node src/download_routersense_data.js <YYYY-MM-DD>
python src/parse_garmin_complete.py
python src/merge_all_data.py
python src/add_derived_features.py
```

For full streaming operations and troubleshooting, see `docs/STREAMING_STACK.md`.
